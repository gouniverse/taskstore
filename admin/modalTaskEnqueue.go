...
func (c *queueManagerController) modalTaskEnqueue(r *http.Request) *hb.Tag {
	taskList, err := c.store.TaskList(taskstore.TaskQuery().
		SetStatus(taskstore.TaskStatusActive).
		SetOrderBy(taskstore.COLUMN_TITLE).
		SetSortOrder(taskstore.DESC))

	if err != nil {
		c.logger.Error("At adminTasks > modalTaskEnqueue", "error", err.Error())
	...
	groupTasks := bs.FormGroup().
		...
		Child(bs.FormSelect().
			...
			Children(lo.Map(taskList, func(task taskstore.TaskInterface, _ int) hb.TagInterface {
				return hb.Option().
					Value(task.ID()).
					Text(task.Title())
			})))
